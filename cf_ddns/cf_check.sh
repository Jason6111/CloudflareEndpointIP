...
if [ ! -f ${CloudflareST} ]; then
	...
	if [[ $get_arch =~ "x86_64" ]];then
	    echo "this is x86_64"
	    wget -P ./cf_ddns/tmp/ ${PROXY}https://github.com/Jason6111/CloudflareEndpointIP/releases/download/$tag/warp-linux-amd64.tar.gz
	    tar -zxf ./cf_ddns/tmp/warp-linux-*.tar.gz -C ./cf_ddns/tmp/
	    mv ./cf_ddns/tmp/CloudflareST ./cf_ddns/
            mv ./cf_ddns/tmp/ip.txt ./cf_ddns/tmp/ipv6.txt ./
	    rm -rf ./cf_ddns/tmp/
	elif [[ $get_arch =~ "aarch64" ]];then
	    echo "this is arm64"
	    wget -P ./cf_ddns/tmp/ ${PROXY}https://github.com/Jason6111/CloudflareEndpointIP/releases/download/$tag/warp-linux-arm64.tar.gz
	    tar -zxf ./cf_ddns/tmp/warp-linux-*.tar.gz -C ./cf_ddns/tmp/
	    mv ./cf_ddns/tmp/CloudflareST ./cf_ddns/
            mv ./cf_ddns/tmp/ip.txt ./cf_ddns/tmp/ipv6.txt ./
	    rm -rf ./cf_ddns/tmp/
	elif [[ $get_arch =~ "386" ]];then
	    echo "this is 386"
	    wget -P ./cf_ddns/tmp/ -N ${PROXY}https://github.com/Jason6111/CloudflareEndpointIP/releases/download/$tag/warp-linux-386.tar.gz
	    tar -zxf ./cf_ddns/tmp/warp-linux-*.tar.gz -C ./cf_ddns/tmp/
	    mv ./cf_ddns/tmp/CloudflareST ./cf_ddns/
            mv ./cf_ddns/tmp/ip.txt ./cf_ddns/tmp/ipv6.txt ./
	    rm -rf ./cf_ddns/tmp/
	    ...
	    echo "this is arm64"
	    wget -P ./cf_ddns/tmp/ ${PROXY}https://github.com/Jason6111/CloudflareEndpointIP/releases/download/$tag/warp-linux-s390x.tar.gz
	    tar -zxf ./cf_ddns/tmp/warp-linux-*.tar.gz -C ./cf_ddns/tmp/
	    mv ./cf_ddns/tmp/CloudflareST ./cf_ddns/
            mv ./cf_ddns/tmp/ip.txt ./cf_ddns/tmp/ipv6.txt ./
	    rm -rf ./cf_ddns/tmp/
	else
	    echo "找不到匹配的CloudflareST程序，请自行下载'https://github.com/Jason6111/CloudflareEndpointIP'，并解压至'./cf_ddns/'文件夹中。"
	    echo "找不到匹配的CloudflareST程序，请自行下载'https://github.com/Jason6111/CloudflareEndpointIP'，并解压至'./cf_ddns/'文件夹中。" > $informlog
	    source $cf_push;
	    exit 1
	fi
fi
